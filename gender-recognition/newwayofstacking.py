...
kres_train = kmeans.labels_
kres_test = kmeans.predict(X_test)
Xr_train = kres_train.reshape(-1, 1)
Xr_test = kres_test.reshape(-1, 1)
...
for name, clf in zip(names, classifiers):
    ...
    y_pred_train = wc * error_min(clf, X_train, best_threshold)
    y_pred_test = wc * error_min(clf, X_test, best_threshold)
...
best_threshold = 0.6
np.savetxt('./result/y_pred_6_n.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.7
np.savetxt('./result/y_pred_7_n.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.8
np.savetxt('./result/y_pred_8_n.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.9
np.savetxt('./result/y_pred_9_n.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')
