...
def error_min(clf, X, threshold):
    ...
    y_pred_test_proba = clf.predict_proba(X)
...
for name, clf in zip(names, classifiers):
    ...
    y_pred_train = error_min(clf, X_train, best_threshold)
    y_pred_test = error_min(clf, X_test, best_threshold)

    Xr_train = np.hstack((Xr_train, y_pred_train.reshape(-1, 1)))
    Xr_test = np.hstack((Xr_test, y_pred_test.reshape(-1, 1)))
    print "end", name
...
for name, clf in zip(names, classifiers):
    print "start ", name
    wc = -1 / learn(clf, Xr_train, y_train, loss)
    w.append(wc)
    print wc
    print "end", name
...
clf.fit(Xr_train, y_train)
print "end VotingClassifier"


print "saving results "
best_threshold = 0.6
np.savetxt('./result/y_pred_6_s.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.7
np.savetxt('./result/y_pred_7_s.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.8
np.savetxt('./result/y_pred_8_s.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')

best_threshold = 0.9
np.savetxt('./result/y_pred_9_s.txt',
           error_min(clf, Xr_test, best_threshold), fmt='%d')
...
for threshold in range(6, 10):
    error.append(compute_pred_score(
        y_train, error_min(clf, Xr_train, threshold / 10)))
...
[('LR', 0.28292787482584925), ('QDA', 8.5993485342019547), ('Neural Net', 13.266331658291458)]
start VotingClassifier
end VotingClassifier
saving results 
0.7
[(6, 0.02181818181818182), (7, 0.01731060606060606), (8, 0.019924242424242424), (9, 0.04653409090909091)]
"""
